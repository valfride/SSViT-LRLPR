...
def make_optimizer(model_param, spec, load_optimizer=False, optimizer=None):
...
class Early_stopping():
    def __init__(self, patience=5, min_delta=0, counter=0, min_validation_loss=np.inf, min_accuracy=0.0):
    ...
    def early_stop(self, validation_loss, accuracy):
        if validation_loss < self.min_validation_loss or accuracy > self.min_accuracy:
        ...
        elif validation_loss > (self.min_validation_loss + self.min_delta) and accuracy < (self.min_accuracy + self.min_delta):
            self.counter += 1
