...
class SVTRNet2DPos(nn.Module):
    ...
    def forward(self, x):
        ...
        # Extract the width to pass down into the blocks
        w = x.shape[-1]
        ...
        # ======================================
        # Block Execution Router
        # ======================================
        def run_block(blk, x, w):
            if isinstance(blk, (Block, DownSConv)):
                # These take (x, w) and return (x, w)
                out = blk(x, w)
                return out[0], out[1]
            elif isinstance(blk, ConvBlock):
                # Takes (x) and returns (x)
                return blk(x), w
            elif isinstance(blk, nn.Sequential):
                # The hidden bug fix: nn.Sequential expects a 4D tensor (B, C, H, W)
                B, N, C = x.shape
                H = N // w
                # 1. Reshape the 3D sequence back into a 2D image map
                x_4d = x.transpose(1, 2).reshape(B, C, H, w)
                # 2. Run the 2D Convolution downsampler
                x_4d = blk(x_4d)
                # 3. Extract the potentially new width and flatten back to a 3D sequence
                new_w = x_4d.shape[-1]
                x_3d = x_4d.flatten(2).transpose(1, 2)
                return x_3d, new_w
            else:
                # Fallback for any unknown blocks
                try:
                    out = blk(x, w)
                    return out[0], out[1] if isinstance(out, tuple) else (out, w)
                except TypeError:
                    return blk(x), w

        # ======================================
        # The Two Official OpenOCR Loops
        # ======================================
        for blk in self.conv_blocks1:
            x, w = run_block(blk, x, w)
        ...
        # Output Routing
        # ======================================
        # IGTR uses last_stage=False. It just reshapes the raw sequence back to 2D!
        if not self.last_stage:
            x = x.reshape(
                [x.shape[0], x.shape[1] // w, w,
                 self.out_channels]).permute(0, 3, 1, 2)
            return x

        # Safety fallback (IGTR doesn't use this branch)
        return x