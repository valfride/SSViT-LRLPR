...
class ViT_Direct_OCR(nn.Module):
    def __init__(self, in_channels=256, d_model=256, num_chars=7, num_classes=37, num_layers=4):
        ...
        # 1. Feature Projector (Tokenizer)
        self.patch_embed = nn.Sequential(
            nn.Conv2d(in_channels, d_model, kernel_size=4, stride=4), 
    ...
    def forward(self, x, layout_logits=None, refine_iters=1):
        """
        refine_iters: How many times to loop. 
                      0 = One Shot. 1 = Read+Refine.
        """
        B = x.size(0)
        
        # 1. Tokenize Image
        vis_tokens = self.patch_embed(x).flatten(2).transpose(1, 2)
        
        # ==========================================================
        # ⚡ NEW: MASKED IMAGE MODELING ("The 50% Cut")
        # ==========================================================
        if self.training:
            # Create a 50% random mask (Bernoulli)
            mask = torch.rand(B, vis_tokens.shape[1], 1, device=x.device) > 0.5
            # Zero out 50% of the patches. The model must hallucinate the rest.
            vis_tokens = vis_tokens * mask.float()
        ...
        # Start with the learned blank queries
        ...
        for i in range(refine_iters + 1):
            ...
            # Combine Queries + Visual Tokens
            tokens = torch.cat([current_queries, vis_tokens], dim=1) 
            
            # Run Transformer
            out_tokens = self.transformer(tokens)
            
            # Extract the answers (the first 7 tokens)
            char_tokens = out_tokens[:, :7]
            
            # FEEDBACK LOOP: The answers become the questions for the next round
            # We do NOT detach here. We want gradients to flow through the refinement process.
            if i < refine_iters:
        ...
        # 5. Output Head
        logits = self.head(final_tokens) # [B, 7, NumClasses]
        
        # 6. Apply Layout Masks
