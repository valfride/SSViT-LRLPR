...
class CustomRoPELayer(nn.Module):
    def __init__(self, d_model, num_heads):
        ...
        # ---> NEW: The "Language Model" (Self-Attention)
        ...
        # ---> UPGRADE: Expanded FFN Bottleneck (d_model * 4) for better memory
    ...
    def forward(self, query, key_value, freqs):
        ...
        # The 7 characters share their hypotheses to enforce license plate grammar
        # ==========================================
        ...
        # The updated characters interrogate the image
        # ==========================================
        q = self.norm1(query)
        kv = key_value 
...
class ViT_CrossAttn_OCR(nn.Module):
    def __init__(self, in_channels=256, d_model=256, num_chars=7, num_classes=37, num_layers=1, num_heads=8):
        super().__init__()
        ...
        self.layers = nn.ModuleList([CustomRoPELayer(d_model, num_heads) for _ in range(num_layers)])
...
class CustomOCR(nn.Module):
    def __init__(self, input_shape=(128, 32, 96), num_classes=37, num_chars=7, d_model=256, num_heads=8):
        ...
        self.stem = nn.Sequential(
            ...
            DeformableProj(128, 128, kernel_size=3, offset_groups=8),
            nn.GroupNorm(8, 128),
            FReLU(128), 
        ...
        self.vit_expert = ViT_CrossAttn_OCR(
            in_channels=256, d_model=d_model, num_chars=num_chars,
    ...
    def forward(self, x, tgt=None, epoch=0, **kwargs):
        ...
        # We no longer extract or return attention maps
