...
class SpatialFeatureExtractor(nn.Module):
    def __init__(self, in_channels=3, feature_dim=128, cnn_heads=4):
        ...
        # We absorb the old coord_proj depth here to keep the parameter count stable.
        self.stem = nn.Sequential(
            nn.Conv2d(in_channels, feature_dim, 3, 1, 1),
            nn.GroupNorm(8, feature_dim),
            HighContrastGate(feature_dim),
            
            # Additional semantic depth replacing the coordinate injection
        ...
        self.body = nn.Sequential(*[RestormerBlock(feature_dim, num_heads=cnn_heads) for _ in range(2)])
    ...
    def forward(self, x): # --- THE FIX: Only accepts x, NO GRIDS ---
...
class Cgnet(nn.Module):
    ...
    def __init__(self, in_channels=3, mode='hybrid', feature_dim=128, cnn_heads=4, d_model=256, vit_heads=8, **kwargs): 
        ...
        if self.mode in ['ocr', 'hybrid']:
            self.student_ocr = CustomOCR.CustomOCR(
                ...
                num_classes=37, 
                num_chars=7, 
                d_model=d_model,     # Now reads from YAML!
                num_heads=vit_heads  # Now reads from YAML!
            )
    ...
    def forward(self, x, temporal_pool=False, return_latent=True, **kwargs): 
        ...
        # --- B. Feature Extraction (Purely Translation Invariant now) ---
